...
import type { ManagerDashboardData } from "@/lib/validators/managerDashboardSchema";

/**
 * Query key for manager dashboard data
...
/**
 * Custom hook for fetching manager dashboard data with React Query
 * 
 * Features:
 ...
 */
export function useManagerDashboardQuery(): UseQueryResult<ManagerDashboardData, Error> & {
  /**
  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      // Fetch dashboard data using the manager service
      // The service layer handles validation and will throw ManagerDashboardValidationError
      // if the API response doesn't match the expected schema
      try {
        return await managerService.getDashboardData();
      } catch (error) {
        // Log error for observability
        if (error instanceof Error) {
          console.error('[useManagerDashboardQuery] Failed to fetch dashboard data:', {
            error: error.message,
            name: error.name,
            stack: error.stack,
